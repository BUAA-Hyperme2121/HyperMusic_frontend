<template>
  ...
  export default {
    ...
    data() {
      return {
        name: "飙升榜",
        cover: "http://p1.music.126.net/pcYHpMkdC69VVvWiynNklA==/109951166952713766.jpg?param=150y150",
        description: "这是HyperMusic飙升榜",
    ...
    methods: {
      ...
      changeStarSongForm(row){
        ...
        starSong(){
            if(this.$store.state.userInfo.isLogin&&this.starSongForm.favorites_id!=''){
                var formData = new FormData();
                formData.append('JWT',JSON.parse(localStorage.getItem("loginInfo")).JWT)
